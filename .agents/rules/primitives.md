...
- **O que NÃO faz:** Não são usadas pelo Frontend React da WorkMate. O frontend usa Server Actions, não fetch para rotas internas.
...
- **O que faz:** Usa `SWR` para buscar dados (read) e cache. Usa `Zustand` ou `useState` para controlar se uma sidebar está aberta ou um botão está em _loading_. Deve expor `isLoading` para disparar os **Shimmer Skeletons** (ver @/.agents/skills/shimmer-skeleton.md).
