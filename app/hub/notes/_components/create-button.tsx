...
export function CreateButton({ activeFolderId, tags }: CreateButtonProps) {
  ...
  const handleFileChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Simulate file upload by reading it as Data URL (Base64) to save locally in mock-db
  ...
  return (
    <>
      ...
      <div ref={menuRef} className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
        ...
        <div
          className={cn(
            "flex flex-col gap-2 mb-2 transition-all duration-300 origin-bottom scale-95 opacity-0 pointer-events-none translate-y-4",
            isOpenMenu && "scale-100 opacity-100 pointer-events-auto translate-y-0"
          )}
        >
          {/* Action item: Enviar PDF */}
          <button
            onClick={() => {
          ...
          >
            <span>Subir PDF</span>
            <div className="flex size-9 items-center justify-center rounded-full bg-linear-to-tr from-orange-500 to-amber-500 text-white">
              <FileText className="size-4" />
            </div>
          </button>

          {/* Action item: Gerenciar Tags */}
          <button
            onClick={() => {
              setIsOpenMenu(false);
              setActiveVault("tag");
          ...
          >
            <span>Tags</span>
            <div className="flex size-9 items-center justify-center rounded-full bg-linear-to-tr from-emerald-500 to-teal-500 text-white">
              <TagIcon className="size-4" />
            </div>
          </button>

          {/* Action item: Criar Pasta */}
          <button
            onClick={() => {
              setIsOpenMenu(false);
          ...
          >
            ...
            <div className="flex size-9 items-center justify-center rounded-full bg-linear-to-tr from-violet-500 to-purple-500 text-white">
              <FolderPlus className="size-4" />
            </div>
          </button>

          {/* Action item: Criar Nota */}
          <button
          ...
          >
            <span>Nova Nota</span>
            <div className="flex size-9 items-center justify-center rounded-full bg-linear-to-tr from-blue-500 to-indigo-500 text-white">
        ...
        <button
          ...
          disabled={isMenuBusy}
          className={cn(
            "flex size-14 items-center justify-center rounded-full bg-primary text-primary-foreground hover:bg-primary/95 transition-all duration-300 cursor-pointer active:scale-95 border-none",
