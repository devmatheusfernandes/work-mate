...
export function ChatInterface({
  ...
  const { messages, isGenerating, sendMessage, clearMessages } = useChatStore();
  ...
  return (
    <div className="flex flex-col h-full w-full bg-background select-none">
      {/* Top Header of Chat */}
      <div className={cn(
        "flex items-center justify-between h-12 border-b border-border/30 shrink-0 bg-background/80 backdrop-blur-md z-10",
        isSidebar ? "pl-11 pr-4" : "px-4"
      )}>
      ...
      <div className="flex-1 flex flex-col justify-between overflow-hidden relative">
        <AnimatePresence mode="wait">
          {!hasMessages ? (
            // EMPTY STATE: Input centered + Suggestions
            <motion.div
              key="empty-state"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              ...
              <div className="w-full max-w-xl flex flex-col items-center gap-6 py-8">
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground max-w-xs">
                    Tire dúvidas sobre seus estudos, organize tarefas e gerencie resumos de forma rápida.
                  </p>
                </motion.div>

                {/* Suggestions Cards (Stack vertically on mobile/sidebar, grid on desktop) */}
                <motion.div 
                  ...
                  className={cn(
                    "grid gap-2 w-full",
                    isSidebar ? "grid-cols-1" : "grid-cols-1 sm:grid-cols-3"
                  )}
                >
                  {SUGGESTIONS.map((s) => (
                    <button
                      ...
                      className={cn(
                        "item text-left p-3 flex flex-col justify-between gap-2 border border-border/40 bg-card/50 hover:bg-muted/10 transition-all duration-200 cursor-pointer min-h-[80px]",
                        "rounded-xl"
                      )}
                    >
                ...
                {/* Simulated empty space to push input box alignment */}
                <div className="h-10" />
              </div>
            </motion.div>
          ) : (
            // CONVERSATION STATE: Scrollable Messages List
            <motion.div
              key="messages-list"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              ...
              {messages.map((msg) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={cn(
                        "max-w-[85%] rounded-2xl px-4 py-2.5 border text-foreground",
                        isUser
                      ...
                      <div className="break-words select-text">
                        ...
                        {!isUser && isGenerating && msg.content === "" && (
                          <span className="inline-flex gap-1 items-center py-1">
                            <span className="size-1.5 bg-foreground/60 rounded-full animate-bounce delay-75" />
                            <span className="size-1.5 bg-foreground/60 rounded-full animate-bounce delay-150" />
                            <span className="size-1.5 bg-foreground/60 rounded-full animate-bounce delay-300" />
                          </span>
                        )}
                        {!isUser && isGenerating && msg.content !== "" && (
          ...
          <div className="w-full max-w-xl flex flex-col items-center">
            ...
            <motion.div
              ...
              className={cn(
                ...
                // Mobile-first touch target styling
                hasMessages 
                  ? "rounded-2xl p-2 px-3" 
                  : "rounded-full p-2.5 px-4"
              )}
            >
              {/* ChatGPT style attachment trigger ("unglued" based on text state/typing/messages) */}
              <motion.button
                layout
                ...
                className={cn(
                  ...
                  // Animate sizes and shapes depending on state
              ...
              {/* ChatGPT style send trigger ("unglued" based on text state/typing/messages) */}
              <motion.button
                layout
